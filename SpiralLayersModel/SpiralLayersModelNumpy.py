...
R = 20
L = 19000*.33
kb = 1.38e-23*1e18 #Because we're in nm and not in meters
T = 300
...
def draw_me_a_fit(datax, datay, model):
    ...
    plt.ylabel('Force (pN)') 
    plt.title('Fit of the Spiral-Layers bending model')
    plt.legend()
    plt.show()
    
    return x, y, popt

def draw_everything(datax, datay, model, r, d):
    x, y, params = draw_me_a_fit(datax, datay, model)
    return 0
...
draw_me_a_fit(datax, datay, model)